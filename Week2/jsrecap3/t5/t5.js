...
//MAP
const map = L.map('map').setView([65.192059, 24.945831], 4.49);
...
for(let i = 0; i < restaurants.length; i++) {
  let y = restaurants[i].location.coordinates[0];
  let x = restaurants[i].location.coordinates[1];

  L.marker([x, y]).addTo(map)
  .bindPopup(`<h3>${restaurants[i].name}</h3><p>${restaurants[i].address}</p>`)
